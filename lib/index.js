...
var DemoList = class extends import_coc.BasicList {
  constructor() {
    super();
    this.name = "demo_list";
    this.description = "CocList for coc-extension";
  ...
  async loadItems(context) {
    return [
      {
        label: "coc-extension list item 1",
        data: { name: "list item 1" }
      },
      {
        label: "coc-extension list item 2",
...
async function activate(context) {
  import_coc2.window.showInformationMessage("coc-extension works!");
  context.subscriptions.push(
    import_coc2.commands.registerCommand("coc-extension.Command", async () => {
      import_coc2.window.showInformationMessage("coc-extension Commands works!");
    }),
    import_coc2.listManager.registerList(new DemoList()),
    import_coc2.sources.createSource({
      name: "coc-extension completion source",
      // unique id
      doComplete: async () => {
